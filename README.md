...
```sh
npm install          # Install dev dependencies (TypeScript, esbuild, Firefox types, Vitest)
...
tsconfig.json       TypeScript configuration (strict, skipLibCheck)
vitest.config.ts    Vitest test configuration
package.json        npm project metadata and build/test scripts
src/
  types.ts          Shared TypeScript interfaces and types
  utils.ts          Shared pure utility functions (getDomain, isValidDomain)
  ...
  __tests__/
    ...
    background.test.ts  Unit tests for background logic (mocked browser APIs)
